...
find.indexes.samples <- function(df, genes, samples, stages.levels, n, stage, stage.ind,
                                 sampsize = table(droplevels(stages.levels[samples])))
{
  set.seed(1)
  stage.indexes <- lapply(seq(1:n), function(x)
    {
    z = randomForest(x = df[samples,genes], y = droplevels(stages.levels[samples]),
    ...
    setdiff(stage.ind[[stage]],which(new.indexes == stage & stages.levels == stage))
  })
  return(df.stage.tumor.rep$short.id[stage.indexes])
}

rf.fpqm.1.3.samples <- find.indexes.samples(req.dfs$fpqm, tumor.fpqm.varSelRF.1.3$selected.vars,
                                            Reduce(union,stage.ind[c(1,3)]), stages.levels, 10, 
                                            'stage iii', stage.ind)
...
rf.fpqm.all.genes.samples <- find.indexes.samples(req.dfs$fpqm,all.genes, c(1:260),
                                                  stages.levels, 10, 'stage iii', stage.ind)
...
rf.fpqm <- find.indexes.samples(req.dfs$fpqm, remove.dots(tumor.fpqm.varSelRF$selected.vars),
                                c(1:260), stages.levels, 10, 'stage iii', stage.ind )
